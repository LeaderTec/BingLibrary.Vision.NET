namespace BingLibrary.Vision.Cameras.CameraSDK.DaHeng
{
    public class CFps
    {
        private UInt64 m_nFrameCount = 0;                           ///< 从上次计算完毕开始累积的帧数
        private double m_dBeginTime = 0.0;                         ///< 第一帧之前的一帧的时间（初始为0）
        private double m_dEndTime = 0.0;                         ///< 最后一帧的时间
        private double m_dFps = 0.0;                         ///< 通过帧数与时间间隔之比得出的帧率(帧/秒)
        private double m_dCurrentFps = 0.0;                         ///< 当前的帧率，可能是预测得到的（帧/秒）
        private UInt64 m_nTotalFrameCount = 0;                           ///< 累积的帧数
        private CStopWatch m_objTime = new CStopWatch();            ///< 计时器

...
namespace BingLibrary.Vision.Cameras
{
    internal class DaHengCamera<T> : BaseCamera<T>
    {
        ...
        private bool m_bIsOpen = false;                                     ///< 相机打开标识
        private bool m_bIsSnap = false;                                     ///< 相机开始采集标识
        private bool m_bColorFilter = false;                                ///< 标识是否支持Bayer格式
        private bool m_bAwbLampHouse = false;                               ///< 标示是否支持光源选择
        private bool m_bWhiteAutoSelectedIndex = true;                      ///<白平衡列表框转换标志
        ...
        private IImageProcessConfig m_objCfg = null;                        ///<图像配置参数对象
        private GxBitmap m_objGxBitmap = null;                              ///<图像显示类对象
        private string m_strPixelColorFilter = null;                        ///<Bayer格式
        private string m_strBalanceWhiteAutoValue = "Off";                  ///<自动白平衡当前的值
        private bool m_bEnableColorCorrect = false;                         ///<颜色校正使能标志位
        private bool m_bEnableGamma = false;                                ///<Gamma使能标志位
        private bool m_bEnableSharpness = false;                            ///<锐化使能标志位
        private bool m_bEnableAutoWhite = false;                            ///<自动白平衡使能标志位
        private bool m_bEnableAwbLight = false;                             ///<自动白平衡光源使能标志位
        private bool m_bEnableDenoise = false;                              ///<图像降噪使能标志位
        private bool m_bEnableSaturation = false;                           ///<饱和度使能标志位
        private bool m_bEnumDevices = false;                                ///<是否枚举到设备标志位
        private List<IGXDeviceInfo> m_listGXDeviceInfo;                     ///<存放枚举到的设备的容器
